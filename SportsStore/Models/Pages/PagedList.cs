...
namespace SportsStore.Models.Pages
{
    ...
    {
        public PagedList(IQueryable<T> query, QueryOptions options)
        {
            CurrentPage = options.CurrentPage;
            PageSize = options.PageSize;
            Options = options;

            if (!IsNullOrEmpty(options.OrderPropertyName))
            {
                query = Order(query, options.OrderPropertyName,
                    options.DescendingOrder);
            }
            if (!IsNullOrEmpty(options.SearchPropertyName)
                && !IsNullOrEmpty(options.SearchTerm))
            {
